...
public class CavalryAction implements UnitAction {
    @Override
        ...
        for (int i = 0; i < mp; i++) {
            ...
            if (check == 0) {
                if (unit.getFaction() == Faction.RED) {
                    if (pos + length < size) {
                        if (fs.get(pos + length, field).getUnit() == null && fs.get(pos + length, field).getType() == UnitType.CAVALRY) {
                            fs.moveUnit(pos, pos + length, field);
                            continue;
                        }
                    }
                    if (pos + 1 < size && (pos) % length != 0) {
                        if (fs.get(pos + 1, field).getUnit() == null && fs.get(pos + 1, field).getType() == UnitType.CAVALRY) {
