        ...
        for (int i = 0; i < mp; i++) {
            ...
            if (check == 0) {
                if (unit.getFaction() == Faction.RED) {
                    if (pos + length < size) {
                        if (field.get(pos + length).getUnit() == null && field.get(pos + length).getType() == UnitType.CAVALRY) {
                            field.moveUnit(pos, pos + length);
                            continue;
                        }
                    }
                    if (pos + 1 < size && (pos) % length != 0) {
                        if (field.get(pos + 1).getUnit() == null && field.get(pos + 1).getType() == UnitType.CAVALRY) {
